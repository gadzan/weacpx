...
- 点击实例树 `+ new session` 打开一个弹窗（取代原先简陋的内联三输入框）。打开时经
  `instances.loadFormOptions(instanceId)` 拉取该实例的 `control.agents.list` + `control.workspaces.list`。
- **尽量选而非输**：agent 与 workspace 都是下拉框（来自实例已配置项）；workspace 下拉额外有
  「+ New workspace…」项，选中后展开 name+path(+desc) 三个输入，提交时先 `control.workspaces.create`
  持久化新工作空间，再 `control.sessions.create` 在其中建会话。
