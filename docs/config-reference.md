...
| `maxParallelTasksPerAgent` | `number` | 否 | `3` | 每个 agent 可同时运行的并行委派任务上限（整数 ≥ 1），跨所有 coordinator 和工作区全局计数。超出上限的 `parallel: true` 任务创建为 `queued` 状态，不占用 acpx session；有 slot 释放时自动按创建时间顺序升为 `running` 并派发。`queued` 任务仍计入 `maxPendingAgentRequestsPerCoordinator` 配额 |
