...
- `xacpx mcp-stdio`：
  ...
  - 运行 MCP server：[runWeacpxMcpServer()](../../src/mcp/weacpx-mcp-server.ts)
...
- server 初始化与 tool registry 缓存：[weacpx-mcp-server.ts](../../src/mcp/weacpx-mcp-server.ts)
- identity 解析（coordinatorSession/sourceHandle 或 resolveIdentity）：[weacpx-mcp-server.ts](../../src/mcp/weacpx-mcp-server.ts)
- 连接 stdio transport：[weacpx-mcp-server.ts](../../src/mcp/weacpx-mcp-server.ts)
