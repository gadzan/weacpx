...
- **Reset GC:** `purgeExpiredResetCoordinators` and friends are intentionally left intact to clean any reset-suffixed records created before this fix; they simply find no new candidates going forward (the machinery is now largely dormant and could be removed in a later cleanup).

---

## Addendum — additional bridge sites found during holistic review

The original tasks (1–6) normalized the obvious coordinator-identity sites. Holistic end-to-end review then surfaced four more places where a raw `session.transportSession`/`transport_session` still bridged to the now-stable coordinator id. Each was the same one-line normalization with a TDD regression test; all were fixed (commits `ea43dbe`, `ee32185`, `0860a77`, `f6594cd`):

- **Task 7 — `orchestration-service.ts resolveRpcSourceContext` (Critical regression).** The RPC source lookup matched `session.transport_session === sourceHandle` raw; with the stable baked id this broke **new agent delegations** right after `/clear`. Also normalized the two `session-handler.ts` coordinator-identity sites (`recordCoordinatorRouteContext` route write, `buildCoordinatorPrompt` interactive result injection).
- **Task 8 — `scheduled-route-create.ts:67` attach guard.** `session.transportSession !== coordinatorSession` threw a false "no longer attached" on `scheduled_create` after `/clear`.
- **Task 9 — `cli.ts prepareMcpCoordinatorStartup`.** The internal-vs-external classification `find` matched raw `transport_session === coordinatorSession`; after `/clear` it misclassified the internal coordinator as **external** and dropped `internalSessionTools`.
- **Task 10 — `orchestration-service.ts listSessionBlockingTasks` / `purgeSessionReferences`.** `/session remove` passed the raw `session.transportSession`; after `/clear` the active-task safety guard missed pre-`/clear` tasks (allowing removal of a session with live work) and the purge left durable orphans. Worker-session matching is unaffected (worker names carry no `:reset-` suffix, so the helper is a no-op on them).

**Sweep conclusion:** an exhaustive `src/` sweep confirms the remaining `transport_session`/`coordinatorSession` comparisons are all (i) transport-vs-transport, (ii) coordinator-vs-coordinator filters that are forward-consistent because both sides are stable post-fix, (iii) external-coordinator-only (external ids never carry `:reset-`), or (iv) the reset-GC machinery (intentionally raw). No remaining raw-vs-stable bridge exists in the delegate / cancel / wake / scheduled / internal-tools / session-remove paths.

**Residual accepted limitations:** (a) legacy reset-suffixed records created *before* this fix are transiently invisible to raw read-filters (`task_get`/`watchTask`/`getGroupSummary`/`listPending`) until GC'd — benign/transient; (b) `/session attach` to a custom (non-`workspace:alias`) transport name then `/clear` is not unified.
