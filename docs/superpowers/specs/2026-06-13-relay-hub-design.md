...
- 外联 WSS：首连用一次性配对 token 注册，换发长期实例凭证存入 config；断线指数退避重连；
- CLI provider：`xacpx relay connect <wss-url> <pairing-token>` 完成配对与频道配置写入。

### 4.4 `packages/relay` —— hub 服务端

- TypeScript，Bun 构建、Node 运行时兼容；HTTP + WS 服务（框架选型在实施计划中定，倾向轻量如 Hono + ws）；
...
- `accounts`：`id, username, password_hash(argon2), role(admin|member), created_at`；
...
- `messages`：`instance_id, session_alias, direction, text, created_at`（回显缓存，按条数/天数保留）；
- `web_sessions`：登录态 token 哈希。

## 6. 实例配对流程

1. Web 看板「添加实例」→ relay 生成一次性配对 token（短有效期，hash 存库）；
2. 用户在 xacpx 侧执行 `xacpx relay connect wss://hub.example.com <token>`；
3. 连接器首连注册，relay 校验后换发长期实例凭证（hash 存库），配对 token 作废；后续重连用实例凭证。

## 7. 安全

- 实例连接生产环境强制 WSS；relay 自身不执行任何命令，仅路由——被攻破时攻击面限于向已配对实例发指令，实例侧仍受 xacpx 自身权限语义约束；
- Web 登录限流；密码 argon2；所有 token/凭证哈希落盘；
...
3. **阶段三**：`relay-web` 三栏看板（会话树 + 对话流）；
4. **阶段四**：任务面板（定时/编排）、设置页、错误恢复打磨与文档。
