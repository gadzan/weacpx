...
The public plugin API will export `ScheduledChannelMessageInput` so plugins can implement `MessageChannelRuntime.sendScheduledMessage` without depending on private core paths.
...
7. For `response.media`, reuse the existing outbound Feishu media path handling where practical; if that reuse is too tangled with active inbound state, log unsupported media and keep text delivery reliable.
...
6. Call `this.agent.chat` with:
   ...
   - `text`: `input.promptText`
   - `replyContextToken`: `input.replyContextToken` when present
   - `abortSignal`: `input.abortSignal` when present, or the channel abort signal as fallback
...
Scheduled Yuanbao turns do not send reply heartbeats in the first implementation because they are not tied to a fresh inbound Yuanbao user message with sender context.
...
If the notice succeeds but the agent turn throws, each plugin should best-effort send a short failure text back to the same route, then rethrow so the scheduler records failure.
...
- Feishu scheduled turn passes `scheduledSessionAlias` metadata and uses `replyContextToken` when available.
- Yuanbao channel implements `sendScheduledMessage` and sends notice plus final response through the gateway.
- Yuanbao scheduled turn uses the outbound queue/chunking path and passes `scheduledSessionAlias` metadata.
...
- Core/package version files are synchronized to `0.5.0`.
