...
weacpx 可以同时启动多个消息频道。目前内置：

- `weixin`：微信频道，使用扫码登录。

插件提供：

- `feishu`：飞书频道，由 `@ganglion/weacpx-channel-feishu` 提供，使用飞书自建应用的 `App ID` / `App Secret`。
- `yuanbao`：元宝频道，由 `@ganglion/weacpx-channel-yuanbao` 提供，使用 `appKey` / `appSecret`，内置元宝签名与 WebSocket gateway。
...
频道配置写在 `~/.weacpx/config.json` 的 `channels[]` 里。推荐用 CLI 管理，不手改 JSON：

```bash
weacpx channel list
weacpx channel show <type>
weacpx channel add <type>
weacpx channel rm <type>
weacpx channel enable <type>
weacpx channel disable <type>
```
...
如果配置里没有显式 `channels[]`，weacpx 会按旧配置自动生成一个启用的微信频道。
...
Feishu is provided by the first-party plugin package `@ganglion/weacpx-channel-feishu`. Install the plugin before adding the channel:
...
```bash
weacpx channel add feishu --restart
```

如果不想立即重启：

```bash
weacpx channel add feishu --no-restart
```

### 4. 在飞书里使用

把机器人加入会话后，在飞书里发送和微信相同的 weacpx 命令：
...
元宝由一方插件包 `@ganglion/weacpx-channel-yuanbao` 提供（配置解析、元宝签名、WebSocket、消息收发、chatKey 路由、agent 调用、任务通知）。

```bash
weacpx plugin add @ganglion/weacpx-channel-yuanbao
weacpx channel add yuanbao --app-key <key> --app-secret <secret>
weacpx restart
```
...
weacpx channel add yuanbao --account main \
    --app-key yb_main --app-secret secret_main

# 再加一个 bot
weacpx channel add yuanbao --account ops \
    --app-key yb_ops --app-secret secret_ops --require-mention false

# 查看某个账号的解析后摘要（appSecret 始终脱敏）
weacpx channel show yuanbao --account main

# 临时下线 / 重新启用
weacpx channel disable yuanbao --account ops
weacpx channel enable  yuanbao --account ops

# 移除某个 bot；最后一个 *enabled* 账号被移除时整个 yuanbao channel 也会被删，
# 但只有"还有别的启用频道"时才会放行；剩余账号全 disabled 同样会被拦下。
weacpx channel rm yuanbao --account ops
```

升级路径与飞书一致：旧的扁平单 bot 配置在第一次 `weacpx channel add yuanbao --account <id> ...` 时会被自动迁移成 `accounts.default = {...旧的 per-bot 字段}` + 新账号。具体规则：
...
```bash
weacpx channel rm feishu
weacpx restart
```

weacpx 不允许删除或禁用最后一个启用的频道，避免 daemon 启动后没有任何消息入口。
...
`weacpx channel add/rm/enable/disable/show feishu --account <id>` 支持多 bot 全流程，无需手编 `config.json`：

```bash
# 1) 第一次添加 bot：会自动创建 feishu channel
weacpx channel add feishu --account main \
    --app-id cli_main --app-secret secret_main

# 2) 再加一个 bot
weacpx channel add feishu --account ops \
    --app-id cli_ops --app-secret secret_ops --require-mention false

# 3) 看某个账号的解析后摘要（appSecret 始终脱敏）
weacpx channel show feishu --account ops

# 4) 临时下线某个 bot 而不删配置
weacpx channel disable feishu --account ops
weacpx channel enable  feishu --account ops

# 5) 移除某个 bot；最后一个 *enabled* 账号被移除时整个 feishu channel 也会被删，
#    但只有"还有别的启用频道"时才会放行；剩余账号全 disabled 同样会被拦下。
weacpx channel rm feishu --account ops
```

升级路径：如果 `~/.weacpx/config.json` 里已经有一个**扁平**的单 bot 配置（顶层直接写 `appId/appSecret`），第一次执行 `weacpx channel add feishu --account <id> ...` 时 CLI 会把旧配置自动迁移成 `accounts.default = {...旧的 per-bot 字段}`，并把新账号挂在 `accounts.<id>` 下。具体规则：
...
把下面 JSON 写入 `~/.weacpx/config.json`，改完执行 `weacpx restart`：
...
排错时一律用 `weacpx channel show feishu` 看每个账号是否被识别（`appSecret` 仍然脱敏为 `***`）。
...
**被策略拒绝的消息**：weacpx 静默丢弃（不回复"无权限"），仅在 `~/.weacpx/runtime/app.log` 里以 `feishu.message.policy_denied` 记录 `accountId/messageId/chatType/senderOpenId/reason`，方便事后审计。Reason 取值：`dm_disabled`、`group_disabled`、`sender_not_allowlisted`、`missing_sender_id`。

---

## 密钥安全

当前版本会把飞书/元宝 `appSecret` 保存在本机 `~/.weacpx/config.json` 中。

建议：

- 优先使用交互式 `weacpx channel add feishu` / `weacpx channel add yuanbao` 输入密钥。
...
```bash
weacpx channel rm feishu --no-restart
weacpx channel add feishu --require-mention false
weacpx restart
```
...
```bash
weacpx channel add feishu --app-id cli_xxx --app-secret your_secret
```
...
- **频道**（`channel`）：管理 `~/.weacpx/config.json` 里的 `channels[]`，决定 daemon 启动哪些频道。

先装插件，再加频道；先 `channel rm`，再 `plugin remove`。

### 插件存放在哪

- 默认目录：`~/.weacpx/plugins/`，里面有一个独立的 `package.json` + `node_modules/`，**不污染**全局或当前项目。
- 自定义：导出环境变量 `WEACPX_PLUGIN_HOME=/some/path`，所有 `weacpx plugin *` 命令都会切到这个目录。
...
# 从 npm 装指定版本
weacpx plugin add @ganglion/weacpx-channel-feishu --version 0.2.1

# 从本地路径装（开发自家插件、调试 fork 时常用）
weacpx plugin add ./packages/channel-my
weacpx plugin add /absolute/path/to/plugin-dir
```
...
| 参数 | 含义 |
| --- | --- |
| `--version <semver>` | 锁定版本；写入 `~/.weacpx/config.json` 的 `plugins[].version`。 |
| `--restart` | 安装后立即重启 daemon。 |
| `--no-restart` | 不重启；变更下次 `weacpx restart` 后生效。 |
| 默认（什么都不传） | 在交互终端里询问"现在重启？"；非交互环境里默认不重启。 |

安装时 weacpx 会立刻 `import()` 一次新装的包跑校验：`apiVersion === 1`、`name` 与包名一致、`type` 字段非空且不含 `:`、单插件内 `type` 不重复、`factory` 存在。校验失败会**立刻报错**，不会写 config，方便回滚。

> **跨插件 `type` 冲突**（A 插件与 B 插件都声明同一个 `type`）不在安装时发现——它依赖另一插件已被 import 才能比较。冲突会在 `weacpx plugin doctor` 或 daemon 启动时由 `registerChannelPlugin` 抛出来。安装新插件后**强烈建议**先 `weacpx plugin doctor`。

### 升级

```bash
# 升级单个插件到 npm registry 的最新版本
weacpx plugin update @ganglion/weacpx-channel-feishu

# 锁定到指定版本
weacpx plugin update @ganglion/weacpx-channel-feishu --version 0.3.0

# 一次升级所有已配置插件
weacpx plugin update --all
```
...
升级流程内部：先 `bun add <pkg>[@<ver>]` / `npm install <pkg>[@<ver>]`，再重新 import + validate；校验失败会报错且**不会**改 config 里的版本号——你的 daemon 重启时仍按旧版本运行（前提：旧版本仍在 node_modules 里；如果包管理器已经覆盖到新版本，你可以再 `weacpx plugin update <pkg> --version <旧版本>` 显式回滚）。

> ⚠️ 升级**只**改磁盘。运行中的 daemon 已经把旧代码 import 进内存，必须 `weacpx restart` 才能加载新版本。
...
# 输出：
# 插件：
# - @ganglion/weacpx-channel-feishu@0.2.1 (enabled)
# - @ganglion/weacpx-channel-yuanbao (enabled)
```

`@<version>` 仅在用 `--version` 锁定过的项目上显示；空版本表示跟随 npm 最新。当前没有 `plugin show` 子命令，详细信息看 `~/.weacpx/config.json` 的 `plugins[]` 数组。

### 启用 / 禁用

```bash
weacpx plugin disable @ganglion/weacpx-channel-feishu
weacpx plugin enable @ganglion/weacpx-channel-feishu
```
...
```bash
weacpx plugin remove @ganglion/weacpx-channel-feishu
# 或简写：
weacpx plugin rm @ganglion/weacpx-channel-feishu
```

会从 plugin home 里把 npm 包卸了（`bun remove` / `npm uninstall`），同时把 `plugins[]` 里的项删除。

> ⚠️ 卸载插件**之前**应该先 `weacpx channel rm <type>`，否则 daemon 重启时会因为找不到提供方而报 `channel X is configured but no enabled plugin provides it`。

### 健康检查

```bash
weacpx plugin doctor                # 检查所有插件
weacpx plugin doctor @scope/xxx     # 只检查一个
```
...
| 情形 | 等级 | 含义 / 行动 |
| --- | --- | --- |
| `package not installed in plugin home` | ERROR | 配置里有但 npm 包没装。重 `weacpx plugin add`。 |
| `failed to import plugin: ...` | ERROR | 装上了但 import 失败。看错误栈，多半是依赖错版或缺 dist。 |
| `unsupported plugin apiVersion` | ERROR | 插件 API 与 weacpx 不匹配。等插件作者升级，或暂时 `disable`。 |
...
升级 / 安装新插件后**强烈建议**先 `weacpx plugin doctor` 再 `weacpx restart`，避免 daemon 起来后才发现配置坏掉。
...
weacpx channel add feishu        # 会按提示输入凭据
weacpx restart
...
**`weacpx plugin update --all` 跑过之后 daemon 启动失败怎么办？**

最常见是新版本插件改了 options 校验或字段。先 `weacpx plugin doctor` 看 ERROR 行；如果是某个具体插件的 import 失败，可以单独把它降级回老版本：

```bash
weacpx plugin update @ganglion/weacpx-channel-feishu --version 0.1.0
weacpx restart
```

**手工改了 `~/.weacpx/plugins/node_modules/...` 后 weacpx 没察觉？**

weacpx 不会去监听文件变化，重启 daemon 才会重新 import。`weacpx plugin update` / `add` / `remove` 都会顺带把 `~/.weacpx/plugins/package.json` 的 `dependencies` 改对，如果你绕过 weacpx 直接动 node_modules，下次 `weacpx plugin doctor` 大概率会报 `package not installed in plugin home`。建议都通过 weacpx CLI 走。

**装本地路径插件后改了源码，daemon 看不到改动？**

`weacpx plugin add ./path` 把本地路径作为 npm dependency 解析（bun add `./path` 创建一个 link，npm install 复制一份）。改完源码后：

- 用 bun：通常已经是 symlink，重新跑 `bun run build` 后 `weacpx restart` 即可。
- 用 npm：需要重跑 `weacpx plugin add ./path` 让 npm install 重新复制。
