...
- `xacpx mcp-stdio`：
  ...
  - 运行 MCP server：[runWeacpxMcpServer()](../src/mcp/xacpx-mcp-server.ts#L132-L149)
...
- server 初始化与 tool registry 缓存：[xacpx-mcp-server.ts](../src/mcp/xacpx-mcp-server.ts#L39-L111)
- identity 解析（coordinatorSession/sourceHandle 或 resolveIdentity）：[xacpx-mcp-server.ts](../src/mcp/xacpx-mcp-server.ts#L113-L130)
- 连接 stdio transport：[xacpx-mcp-server.ts](../src/mcp/xacpx-mcp-server.ts#L132-L149)
