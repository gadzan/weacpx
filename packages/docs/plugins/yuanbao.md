...
- On completion, its **final answer is stored** and a short ping is sent to the active chat: `✅ <alias> completed — /use <alias> to view result` (or `⚠️ <alias> failed — /use <alias> to view details`). Under `config.language: "zh"` these pings appear in Chinese (`✅ <alias> 已完成，/use <alias> 查看结果`).
