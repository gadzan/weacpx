...
import { getPluginVersion, getOpenclawVersion, getOperationSystem } from "../env.js";
import { createYuanbaoLog as createLog } from "../logger.js";
...
export class YuanbaoWsClient {
  ...
  sendAndWait(
    ...
    return new Promise<WsSendMessageResponse>((resolve, reject) => {
      ...
      this.pendingRequests.set(msgId, { resolve: resolve as (resp: unknown) => void, reject, timer });
  ...
  private doConnect(): void {
    ...
    try {
      ...
      ws.on("close", (code: number, reason: Buffer) => {
        ...
        this.stopHeartbeat();
        this.rejectPendingRequests(new Error(`WebSocket closed: code=${code}, reason=${reasonStr}`));
