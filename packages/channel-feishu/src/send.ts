...
import { createReadStreamForFeishu, inferFeishuFileType, type FeishuMediaClient } from "./media.js";
...
export async function sendTextFeishu(input: {
  ...
  if (input.replyToMessageId) {
    const response = await input.client.im.message.reply({
...
export async function sendMediaFeishu(input: {
  ...
  if (input.media.kind === "image") {
    const upload = await input.client.im.image!.create({
  ...
  const upload = await input.client.im.file!.create({
    data: {
...
async function sendRawMediaMessage(
  ...
  if (replyToMessageId) {
    const response = await client.im.message!.reply({ path: { message_id: replyToMessageId }, data: { msg_type: msgType, content } });
