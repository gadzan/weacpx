...
test("readVersion resolves installed-package layout (dist/cli.js → ../package.json)", async () => {
  ...
  try {
    await writeFile(join(root, "package.json"), JSON.stringify({ name: "weacpx", version: "9.9.9" }));
...
test("readVersion resolves dev/source layout (src/version.ts → ../package.json)", async () => {
  const root = await createWorkspace();
  try {
    await writeFile(join(root, "package.json"), JSON.stringify({ name: "weacpx", version: "1.2.3" }));
...
test("readVersion uses dev fallback when first candidate is missing", async () => {
  const root = await createWorkspace();
  try {
    await writeFile(join(root, "package.json"), JSON.stringify({ name: "weacpx", version: "2.0.0" }));
