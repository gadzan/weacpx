...
test("plugin list prints configured plugins", async () => {
  ...
  expect(code).toBe(0);
  expect(harness.lines).toEqual(["插件：", "- weacpx-channel-demo@1.0.0 (enabled)"]);
});

test("plugin add installs, validates, saves config, and does not enable channel", async () => {
  const harness = createHarness(baseConfig());

  const code = await handlePluginCli(["add", "weacpx-channel-demo", "--version", "^1.2.0"], harness.deps);

  expect(code).toBe(0);
  expect(harness.calls).toEqual(["install:weacpx-channel-demo:^1.2.0"]);
  expect(harness.getConfig().plugins).toEqual([{ name: "weacpx-channel-demo", version: "^1.2.0", enabled: true }]);
  expect(harness.getConfig().channels).toEqual([{ id: "weixin", type: "weixin", enabled: true }]);
  expect(harness.lines).toContain("插件 weacpx-channel-demo 已安装");
  expect(harness.lines).toContain("提供频道：demo");
});

test("plugin rm removes plugin config and package", async () => {
  const harness = createHarness(baseConfig({ plugins: [{ name: "weacpx-channel-demo", enabled: true }] }));

  const code = await handlePluginCli(["rm", "weacpx-channel-demo"], harness.deps);

  expect(code).toBe(0);
  expect(harness.calls).toEqual(["remove:weacpx-channel-demo"]);
...
test("plugin disable and enable toggle config", async () => {
  ...
  expect(await handlePluginCli(["disable", "weacpx-channel-demo"], harness.deps)).toBe(0);
  expect(harness.getConfig().plugins[0]?.enabled).toBe(false);

  expect(await handlePluginCli(["enable", "weacpx-channel-demo"], harness.deps)).toBe(0);
  expect(harness.getConfig().plugins[0]?.enabled).toBe(true);
});

test("plugin rm refuses when a configured channel depends on it", async () => {
  const harness = createHarness(baseConfig({
    plugins: [{ name: "@ganglion/weacpx-channel-yuanbao", enabled: true }],
    channels: [
      { id: "weixin", type: "weixin", enabled: true },
      { id: "yuanbao", type: "yuanbao", enabled: true },
    ],
  }));
  harness.summaries.set("@ganglion/weacpx-channel-yuanbao", {
    name: "@ganglion/weacpx-channel-yuanbao",
    channels: ["yuanbao"],
  });

  const code = await handlePluginCli(["rm", "@ganglion/weacpx-channel-yuanbao"], harness.deps);

  expect(code).toBe(1);
  expect(harness.calls).toEqual([]);
  expect(harness.getConfig().plugins).toEqual([{ name: "@ganglion/weacpx-channel-yuanbao", enabled: true }]);
  expect(harness.getConfig().channels).toEqual([
    { id: "weixin", type: "weixin", enabled: true },
    { id: "yuanbao", type: "yuanbao", enabled: true },
  ]);
  expect(harness.lines.some((line) => line.includes("yuanbao") && line.includes("weacpx channel rm"))).toBe(true);
});
...
test("plugin disable refuses when a configured channel depends on it", async () => {
  ...
  const code = await handlePluginCli(["disable", "@ganglion/weacpx-channel-yuanbao"], harness.deps);
...
test("plugin rm proceeds when no configured channel depends on the plugin", async () => {
  ...
  expect(code).toBe(0);
  expect(harness.calls).toEqual(["remove:@ganglion/weacpx-channel-yuanbao"]);
  expect(harness.getConfig().plugins).toEqual([]);
});
...
test("plugin rm refuses when validation fails and non-weixin channels are configured", async () => {
  ...
  harness.validateErrors.set("@ganglion/weacpx-channel-yuanbao", new Error("plugin module missing"));
...
test("plugin remove is the canonical alias for plugin rm", async () => {
  ...
  const code = await handlePluginCli(["remove", "weacpx-channel-demo"], harness.deps);

  expect(code).toBe(0);
  expect(harness.calls).toEqual(["remove:weacpx-channel-demo"]);
  expect(harness.getConfig().plugins).toEqual([]);
  expect(harness.lines).toContain("插件 weacpx-channel-demo 已移除");
});

test("plugin update updates one configured plugin and preserves enabled state", async () => {
  const harness = createHarness(baseConfig({ plugins: [{ name: "weacpx-channel-demo", version: "^1.0.0", enabled: false }] }));

  const code = await handlePluginCli(["update", "weacpx-channel-demo", "--version", "^2.0.0"], harness.deps);

  expect(code).toBe(0);
  expect(harness.calls).toEqual(["update:weacpx-channel-demo:^2.0.0"]);
  expect(harness.getConfig().plugins).toEqual([{ name: "weacpx-channel-demo", version: "^2.0.0", enabled: false }]);
  expect(harness.lines).toContain("插件 weacpx-channel-demo 已更新");
  expect(harness.lines).toContain("提供频道：demo");
});
...
test("plugin update rolls back to previous pinned version when validation fails", async () => {
  const harness = createHarness(baseConfig({ plugins: [{ name: "weacpx-channel-demo", version: "^1.0.0", enabled: true }] }));
  harness.validateErrors.set("weacpx-channel-demo", new Error("broken plugin export"));

  const code = await handlePluginCli(["update", "weacpx-channel-demo", "--version", "^2.0.0"], harness.deps);

  expect(code).toBe(1);
  expect(harness.calls).toEqual([
    "update:weacpx-channel-demo:^2.0.0",
    "update:weacpx-channel-demo:^1.0.0",
  ]);
  expect(harness.getConfig().plugins).toEqual([{ name: "weacpx-channel-demo", version: "^1.0.0", enabled: true }]);
  expect(harness.lines).toContain("插件 weacpx-channel-demo 更新后校验失败：broken plugin export");
...
test("plugin update without prior pinned version surfaces no-rollback hint", async () => {
  ...
  expect(code).toBe(1);
  expect(harness.calls).toEqual(["update:weacpx-channel-demo:^2.0.0"]);
  expect(harness.lines).toContain("无法自动回滚（weacpx-channel-demo 未锁定先前版本）；请手动 weacpx plugin add weacpx-channel-demo 重装。");
});
...
test("plugin add surfaces friendly error and exits 1 when package install fails", async () => {
  ...
  harness.deps.installPackage = async () => {
    throw new Error("npm install weacpx-channel-demo exited with code 1");
  };

  const code = await handlePluginCli(["add", "weacpx-channel-demo"], harness.deps);

  expect(code).toBe(1);
  expect(harness.getConfig().plugins).toEqual([]);
  expect(harness.lines.some((line) => line.startsWith("插件 weacpx-channel-demo 安装失败："))).toBe(true);
...
test("plugin add surfaces friendly error when validation fails", async () => {
  const harness = createHarness(baseConfig());
  harness.validateErrors.set("weacpx-channel-demo", new Error("plugin module missing default export"));

  const code = await handlePluginCli(["add", "weacpx-channel-demo"], harness.deps);

  expect(code).toBe(1);
  expect(harness.getConfig().plugins).toEqual([]);
  expect(harness.lines.some((line) => line.startsWith("插件 weacpx-channel-demo 校验失败："))).toBe(true);
});

test("plugin add surfaces upgrade-weacpx hint when plugin requires newer core", async () => {
  const harness = createHarness(baseConfig());
  harness.validateErrors.set(
    "weacpx-channel-demo",
    new Error("插件 weacpx-channel-demo requires weacpx >=99.0.0; current is 0.3.3; upgrade weacpx"),
  );
  ...
  expect(code).toBe(1);
  expect(harness.getConfig().plugins).toEqual([]);
  expect(harness.lines.some((line) => /upgrade weacpx/i.test(line))).toBe(true);
});
...
test("plugin add surfaces unsupported-apiVersion hint", async () => {
  ...
  harness.validateErrors.set(
    "weacpx-channel-demo",
    new Error("插件 weacpx-channel-demo 使用不支持的 apiVersion 2; supported: 1; 请安装与当前 weacpx 兼容的插件版本 (install a compatible plugin)"),
  );
...
test("plugin rm surfaces friendly error and keeps config when uninstall fails", async () => {
  ...
  harness.deps.removePackage = async () => {
    throw new Error("npm uninstall weacpx-channel-demo exited with code 1");
  };

  const code = await handlePluginCli(["rm", "weacpx-channel-demo"], harness.deps);

  expect(code).toBe(1);
  expect(harness.getConfig().plugins).toEqual([{ name: "weacpx-channel-demo", enabled: true }]);
  expect(harness.lines.some((line) => line.startsWith("插件 weacpx-channel-demo 卸载失败："))).toBe(true);
});
...
test("plugin restart-after-mutation surfaces friendly error when daemon restart throws", async () => {
  ...
  expect(harness.lines).toContain("请查看日志：~/.weacpx/runtime/stderr.log");
  expect(harness.lines).toContain("也可以稍后执行：weacpx start");
});

test("plugin doctor prints structured diagnostics and returns zero without errors", async () => {
  const harness = createHarness(baseConfig({ plugins: [{ name: "weacpx-channel-demo", enabled: true }] }));
  harness.deps.inspectPlugins = async () => [
    { level: "ok", plugin: "weacpx-channel-demo", message: "plugin is installed and valid; channels: demo" },
    { level: "warn", plugin: "weacpx-channel-demo", message: "plugin has no configured channels" },
  ];
  ...
  expect(harness.lines).toEqual([
    "OK weacpx-channel-demo: plugin is installed and valid; channels: demo",
    "WARN weacpx-channel-demo: plugin has no configured channels",
  ]);
});
...
test("plugin doctor returns one when any error is reported", async () => {
  const harness = createHarness(baseConfig({ plugins: [{ name: "weacpx-channel-demo", enabled: true }] }));
  harness.deps.inspectPlugins = async () => [
    { level: "error", plugin: "weacpx-channel-demo", message: "failed to import plugin: module not found" },
  ];

  const code = await handlePluginCli(["doctor", "weacpx-channel-demo"], harness.deps);

  expect(code).toBe(1);
  expect(harness.lines).toEqual([
    "ERROR weacpx-channel-demo: failed to import plugin: module not found",
  ]);
});
...
test("plugin known lists official Feishu and Yuanbao plugin packages", async () => {
  ...
  expect(output).toContain("@ganglion/weacpx-channel-feishu");
  expect(output).toContain("@ganglion/weacpx-channel-yuanbao");
  expect(output).toContain("weacpx plugin add <package>");
